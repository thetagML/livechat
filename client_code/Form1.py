...
class Form1(Form1Template):
  ...
  def __init__(self, **properties):
    ...
    self.drop_down_channel.items = [(channel['name'], channel) for channel in anvil.server.call_s('get_channels')]
    self.get_messages()
  ...
  def get_messages(self):
    self.repeating_panel_1.items = anvil.server.call_s('get_messages', self.drop_down_channel.selected_value)
  ...
  def send_message(self):
    anvil.server.call('add_message', self.drop_down_channel.selected_value, self.text_box_message.text)
    self.get_messages()
